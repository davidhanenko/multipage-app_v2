...
module.exports.updateService = async (req, res) => {
  ...
  try {
    ...
    if (Object.keys(req.files).length !== 0) {
      ...
      // first image
      if (req.files['image1']) {
        if (req.files['image1'][0].size >= 10000000) {
          req.flash(
            'error',
            'Please review size of images. 10Mb is maximum allowed'
          )
      ...
      // second image
      if (req.files['image2']) {
        if (req.files['image2'][0].size >= 10000000) {
          req.flash(
            'error',
            'Please review size of images. 10Mb is maximum allowed'
          )
      ...
      // 3rd image
      if (req.files['image3']) {
        if (req.files['image3'][0].size >= 10000000) {
          req.flash(
            'error',
            'Please review size of images. 10Mb is maximum allowed'
          )
      ...
      // 4th image
      if (req.files['image4']) {
        if (req.files['image4'][0].size >= 10000000) {
          req.flash(
            'error',
            'Please review size of images. 10Mb is maximum allowed'
          )
...
module.exports.deleteService = async (req, res) => {
  try {
    ...
    logger.error('From Services/delete:' + err)
    req.flash('error', err.message)
    res.redirect('back')
  }
}
